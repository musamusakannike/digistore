    ...
    return (
        <div className="glass-card p-6 border border-white/10 h-full">
            <div className="flex items-center justify-between mb-8">
                ...
                <select className="bg-white/5 border border-white/10 text-white text-sm px-3 py-1 focus:outline-none">
            ...
            <div className="h-64 flex items-end justify-between gap-2">
                {data.map((value, index) => (
                    <div key={index} className="flex-1 flex flex-col items-center gap-2 group">
                        ...
                        >
                            {/* Tooltip */}
                            <div className="opacity-0 group-hover:opacity-100 absolute -top-8 left-1/2 -translate-x-1/2 bg-white text-black text-xs font-bold px-2 py-1 transition-opacity">
                                {value}%
                            </div>
                        </div>
